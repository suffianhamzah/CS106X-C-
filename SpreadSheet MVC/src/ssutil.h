...
typedef double (*cmdFormula)(const Vector<double>& values);
/**
 * Type: location
 * --------------
...
struct range {
    std::string startCell, stopCell;
    //range(std::string start, std::string stop) : startCell(start), stopCell(stop) {}
} ;
		
/**
 * Function: stypedef void (*cmdFormula)(const Vector<double>& values);tringToLocation
...
void setUpFTable(Map<std::string, cmdFormula>& ftable);
#endif
