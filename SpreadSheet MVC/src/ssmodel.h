 ...
 * Usage: model.clearAll();
 * ----------------------------------------
 * This member function clears all cell content within the model;
 */
    void clearAll();
    bool isDefined(const std::string& cellname);
    bool isFormula(const std::string& formula);
    Cell* getCell(const std::string& cellname);
    double getRangeVal(const std::string& cellname1, const std::string& cellname2, const std::string& formula);
    
/** 
 * Depedency graph functions
 * Members:
 * addNode(const string &cellname);
 * ----------------------------------------
 * This member function initializes cell in graph;
 *
 * addDependency(const string& parentCell, const string& cellname);
 * checkDependency(const string& cellInsert, const string& parentCell);
 * update
 */
   bool checkCircular(const std::string& parentCell, const std::string& cellname);
   Vector <std::string> cellRange(const std::string& cellname1, const std::string& cellname2);

// void addDependency
private:
    /* For you to decide */
    xy getRowCol(const std::string& cellname) const;
    Grid<Cell*> celldata;
    SSView *view;
    SimpleGraph dGraph;
    Map<std::string, cmdFormula> fTable;
    bool checkCircularHelper(Node* parent, Node* curr, Set<std::string>& visited);
    void addArc(Node* first, Node* second);
    void addDependencies(const std::string& cellname, const Expression* exp);
    void removeDependencies(const std::string& cellname);
    void updateCells(const std::string& cellname);
    void updateCellsHelper(const std::string& cellname, Set<std::string>& isVisited);
    void evalCell(const std::string& cellname, Cell *cellinfo);
    void addNode(const std::string& cellname);
    void connectNode(const std::string& parentCell, const std::string& cellname);
    void addSublist(const std::string& cellname, const std::string& cellname2, int mode);
    Arc* findArc(const std::string& start, const std::string& finish);
    void removeArc(Node* node, Arc* arc);
    void createCell(const std::string& cellname);
    void removeSubList(const std::string& cellname, const std::string& cellname2, int mode);
